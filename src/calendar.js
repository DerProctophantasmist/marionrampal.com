...
(function() {
  var calendarLink, moment, striptags;
...
// FileSaver.saveAs(calendar.toBlob(),'Songkick events')
                for (i = 0, len = newEvents.length; i < len; i++) {
                  ...
                  $http.get(`http://api.geonames.org/timezoneJSON?lat=${newEvent.lat}&lng=${newEvent.lng}&username=proctophantasmist`).then((response) => {
                    newEvent.timezone = response.data.timezoneId;
                    return this.calendar.unshift(newEvent);
                  }).catch(function(error) {
                    console.log(error);
                    return console.log("could not retrieve timzone data forme geonames");
                  });
                }
              });
            }
